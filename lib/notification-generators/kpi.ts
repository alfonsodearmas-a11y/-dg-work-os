  ...
  try {
    ...
    for (const alert of activeAlerts.rows) {
      ...
      try {
        ...
        if (err instanceof NotificationDeliveryError) {
          logger.error(err.toLogContext(), '[kpi-gen] notification delivery failed');
        } else {
          logger.error({ err }, '[kpi-gen] notification delivery failed (unexpected error type)');
        }
      }
    }
    ...
    for (const ag of agencies) {
      try {
        ...
        // Source table may not exist yet — skip silently
      }
    }
  } catch (err) {
