...
¹ The assignee clause is identity-based (`userId === assignee_user_id`) and applies within the route's agency scoping (the scoped `getCase` runs first). AS BUILT: an assignee stranded outside the effective agency by a workbook agency change gets the same opaque 404 as any other-agency manager — no cross-agency access; the owning manager or a superadmin reassigns. Transfers still clear assignments, so the normal path never produces this. The identity clause is defense-in-depth for in-scope assignees, future-proofing a possible non-manager staff role.
