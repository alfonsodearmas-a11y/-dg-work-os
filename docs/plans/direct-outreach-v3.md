...
¹ The assignee clause is identity-based (`userId === assignee_user_id`). AS AMENDED post-v3 (assign-any-human fix): `getCase` visibility is scope OR requester-is-assignee, so the assigned officer can open and work their case even when its effective agency is not theirs (superadmins may assign ANY active human via `/api/direct-outreach/officers`; managers keep the Q3 case-agency picker). Out-of-scope NON-assignees still get the opaque 404. A cross-agency assignee can view and post but not reassign or transfer.
