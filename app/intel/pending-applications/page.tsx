...
export default function PendingApplicationsPage() {
  ...
  // Fetch stats (with retry for slow Turbopack compilation)
  useEffect(() => {
    let cancelled = false;
    async function loadStats(retries = 2) {
      for (let i = 0; i <= retries; i++) {
        try {
          ...
          console.warn('[pending-applications] Stats fetch attempt', i + 1, 'failed:', err);
  ...
  const handleSort = (col: string) => {
    if (sortBy === col) {
      ...
      setSortBy(col);
      setSortOrder(col === 'days_waiting' ? 'desc' : 'asc');
    }
  };
  ...
  // Region breakdown (combined or filtered)
  const regionData = (() => {
    if (!stats) return [];
    const src = agencyFilter === 'GPL' ? [gplStats] :
                agencyFilter === 'GWI' ? [gwiStats] :
                [gplStats, gwiStats];
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="stat-number text-2xl md:text-3xl">
            {combinedTotal > 0 ? Math.round(
              ((gplStats?.avgDaysWaiting || 0) * (gplStats?.total || 0) +
               (gwiStats?.avgDaysWaiting || 0) * (gwiStats?.total || 0)) /
              combinedTotal
        ...
        <div className="card-premium p-4">
          ...
          <div className="mt-2 space-y-1">
            {gplStats?.longestWaitCustomer && (
              <button
                onClick={() => setSelectedRecord(gplStats.longestWaitCustomer)}
                className="block text-xs text-amber-400 hover:text-amber-300 truncate max-w-full text-left"
              >
            ...
            {gwiStats?.longestWaitCustomer && (
              <button
                onClick={() => setSelectedRecord(gwiStats.longestWaitCustomer)}
                className="block text-xs text-cyan-400 hover:text-cyan-300 truncate max-w-full text-left"
              >
                ...
                {regionData.map(r => (
                  <tr key={r.region} className="border-b border-[#2d3a52]/50 hover:bg-[#1a2744]/50">
                    ...
                    <td className="py-2.5 pl-3 text-right">
                      <span className={`${r.pctOver30 > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                        {r.pctOver30}%
                      </span>
                    </td>
                  </tr>
      ...
      <div className="card-premium p-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            <select
              value={waitBracket}
              onChange={e => setWaitBracket(e.target.value as WaitBracket)}
              className="appearance-none pl-3 pr-8 py-2 rounded-lg bg-[#0a1628] border border-[#2d3a52] text-[#94a3b8] text-sm focus:border-[#d4af37] focus:outline-none cursor-pointer"
            >
            ...
            <tbody>
              {loadingRecords ? (
                ...
                <tr
                  ...
                  className="border-b border-[#2d3a52]/50 hover:bg-[#1a2744]/50 cursor-pointer transition-colors"
                >
                  <td className="py-3 px-4">
                    <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                      record.agency === 'GPL' ? 'bg-amber-500/20 text-amber-400' : 'bg-cyan-500/20 text-cyan-400'
                    }`}>
                      {record.agency}
                    </span>
                  </td>
                  ...
                  <td className="py-3 px-3 text-[#94a3b8] hidden lg:table-cell truncate max-w-[200px]">
                    {record.district || record.villageWard || '—'}
                  </td>
                  <td className="py-3 px-3 text-[#94a3b8] whitespace-nowrap">{formatDate(record.applicationDate)}</td>
                  <td className="py-3 px-3">
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold border ${getBadgeColor(record.daysWaiting)}`}>
                      {record.daysWaiting}d
                    </span>
                  </td>
      ...
      {selectedRecord && (
        <>
          <div
            className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[46]"
          ...
          <div className="fixed inset-y-0 right-0 w-full sm:w-[440px] bg-[#0a1628] border-l border-[#2d3a52] z-50 flex flex-col">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  <span className={`px-2.5 py-1 rounded text-xs font-bold ${
                    selectedRecord.agency === 'GPL' ? 'bg-amber-500/20 text-amber-400' : 'bg-cyan-500/20 text-cyan-400'
                  }`}>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5" style={{ WebkitOverflowScrolling: 'touch' }}>
              ...
              {/* Copy Contact Button */}
              <button
                onClick={() => copyContact(selectedRecord)}
...
function SortHeader({ label, field, current, order, onSort }: {
  ...
  return (
    <th
      className="text-left py-3 px-3 cursor-pointer hover:text-white transition-colors select-none whitespace-nowrap"
