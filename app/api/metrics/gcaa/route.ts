...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await transaction(async (client) => {
      ...
      if (existing.rows.length > 0) {
        ...
        metricsResult = await client.query(
          ...
          [activeRegistrations, inspectionsMtd, inspectionsTarget, complianceRate, incidentReports || 0, incidentDetails, renewalsPending || 0, notes, 'dg-admin', reportDate]
        );
        action = 'UPDATE';
      } else {
        metricsResult = await client.query(
          ...
          [reportDate, activeRegistrations, inspectionsMtd, inspectionsTarget, complianceRate, incidentReports || 0, incidentDetails, renewalsPending || 0, notes, 'dg-admin']
        );
        action = 'CREATE';
      }
    ...
    await auditService.log({ userId: 'dg-admin', action: result.action, entityType: 'gcaa_daily_metrics', entityId: result.metrics.id, newValues: body, request });
