...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await transaction(async (client) => {
      ...
      if (existing.rows.length > 0) {
        ...
        metricsResult = await client.query(
          ...
          [currentLoad, capacity, activeOutages || 0, affectedCustomers || 0, avgRestorationTime, collectionRate, hfoGeneration, lfoGeneration, solarGeneration, otherGeneration, notes, user.id, reportDate]
        );
        action = 'UPDATE';
      } else {
        metricsResult = await client.query(
          ...
          [reportDate, currentLoad, capacity, activeOutages || 0, affectedCustomers || 0, avgRestorationTime, collectionRate, hfoGeneration, lfoGeneration, solarGeneration, otherGeneration, notes, user.id]
        );
        action = 'CREATE';
      }
    ...
    await auditService.log({ userId: user.id, action: result.action, entityType: 'gpl_daily_metrics', entityId: result.metrics.id, newValues: body, request });
