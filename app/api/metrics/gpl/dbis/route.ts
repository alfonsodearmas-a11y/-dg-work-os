...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await transaction(async (client) => {
      ...
      if (existing.rows.length > 0) {
        ...
        metricsResult = await client.query(
          ...
          [reportDate, JSON.stringify(stationData), hampshireSolarMwp || 0, prospectSolarMwp || 0, trafalgarSolarMwp || 0, totalFossilCapacity, totalRenewable, totalFossilCapacity + totalRenewable, eveningPeakOnbars, eveningPeakSuppressed, dayPeakOnbars, dayPeakSuppressed, generationAvailability, activeOutages || 0, affectedCustomers || 0, avgRestorationTime, collectionRate, hfoGeneration, lfoGeneration, solarGeneration, otherGeneration, notes, user.id]
        );
        action = 'CREATE';
      }
    ...
    await auditService.log({ userId: user.id, action: result.action, entityType: 'gpl_dbis_daily', entityId: result.metrics.id, newValues: body, request });
