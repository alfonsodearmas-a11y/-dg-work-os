...
const cjiaMetricsSchema = z.object({
  reportDate: z.string().min(1),
  arrivals: z.number(),
  departures: z.number().optional(),
  onTimePercent: z.number().optional(),
  revenueMtd: z.number().optional(),
  revenueTarget: z.number().optional(),
  safetyIncidents: z.number().optional(),
  safetyIncidentDetails: z.string().optional(),
  powerUptime: z.number().optional(),
  baggageUptime: z.number().optional(),
  securityUptime: z.number().optional(),
...
export const POST = withErrorHandler(async (request: NextRequest) => {
  ...
  const { data, error } = await parseBody(request, cjiaMetricsSchema);
  if (error) return error;

  const { reportDate, arrivals, departures, onTimePercent, revenueMtd, revenueTarget, safetyIncidents, safetyIncidentDetails, powerUptime, baggageUptime, securityUptime, notes } = data!;
  ...
  const result = await transaction(async (client) => {
    ...
    if (existing.rows.length > 0) {
      ...
      metricsResult = await client.query(
        ...
        [arrivals, departures, onTimePercent, revenueMtd, revenueTarget, safetyIncidents || 0, safetyIncidentDetails, powerUptime, baggageUptime, securityUptime, notes, session.user.id, reportDate]
      );
      action = 'UPDATE';
    } else {
      metricsResult = await client.query(
        ...
        [reportDate, arrivals, departures, onTimePercent, revenueMtd, revenueTarget, safetyIncidents || 0, safetyIncidentDetails, powerUptime, baggageUptime, securityUptime, notes, session.user.id]
      );
      action = 'CREATE';
    }
  ...
  await auditService.log({ userId: session.user.id, action: result.action, entityType: 'cjia_daily_metrics', entityId: result.metrics.id, newValues: data, request });
