...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await transaction(async (client) => {
      ...
      if (existing.rows.length > 0) {
        ...
        metricsResult = await client.query(
          ...
          [arrivals, departures, onTimePercent, revenueMtd, revenueTarget, safetyIncidents || 0, safetyIncidentDetails, powerUptime, baggageUptime, securityUptime, notes, user.id, reportDate]
        );
        action = 'UPDATE';
      } else {
        metricsResult = await client.query(
          ...
          [reportDate, arrivals, departures, onTimePercent, revenueMtd, revenueTarget, safetyIncidents || 0, safetyIncidentDetails, powerUptime, baggageUptime, securityUptime, notes, user.id]
        );
        action = 'CREATE';
      }
    ...
    await auditService.log({ userId: user.id, action: result.action, entityType: 'cjia_daily_metrics', entityId: result.metrics.id, newValues: body, request });
