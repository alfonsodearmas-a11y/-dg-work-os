...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await transaction(async (client) => {
      ...
      if (existing.rows.length > 0) {
        ...
        metricsResult = await client.query(
          ...
          [nrwPercent, waterProduced, waterBilled, activeDisruptions || 0, disruptionAreas, avgResponseTime, avgRepairTime, customerComplaints || 0, notes, user.id, reportDate]
        );
        action = 'UPDATE';
      } else {
        metricsResult = await client.query(
          ...
          [reportDate, nrwPercent, waterProduced, waterBilled, activeDisruptions || 0, disruptionAreas, avgResponseTime, avgRepairTime, customerComplaints || 0, notes, user.id]
        );
        action = 'CREATE';
      }
    ...
    await auditService.log({ userId: user.id, action: result.action, entityType: 'gwi_daily_metrics', entityId: result.metrics.id, newValues: body, request });
