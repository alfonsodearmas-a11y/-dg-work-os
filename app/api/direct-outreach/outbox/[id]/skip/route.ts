// OP Direct outbox — superadmin skip: pending → skipped (never posted; can be
// re-queued later via retry).

import { NextRequest, NextResponse } from 'next/server';
import { requireRole } from '@/lib/auth-helpers';
import { getOutboxStatus, skipOutboxRow } from '@/lib/direct-outreach/outbox';
...
export async function POST(
  ...
  try {
    if (await skipOutboxRow(id)) return NextResponse.json({ ok: true });
    ...
    logger.error({ err, id }, '[direct-outreach] outbox skip failed');
