...
export function TaskComments({ taskId, users }: TaskCommentsProps) {
  ...
  return (
    <div className="flex flex-col">
      <h3 className="text-xs font-semibold text-[#64748b] uppercase tracking-wider mb-3 px-4 pt-4">
        <MessageSquare className="h-3.5 w-3.5 inline-block mr-1.5 -mt-0.5" />
        Comments
        {comments.length > 0 && (
          <span className="ml-1.5 text-[#94a3b8]">{comments.length}</span>
        )}
      </h3>
      ...
      >
        {loading ? (
          ...
          <p className="text-xs text-[#64748b] italic py-2">No comments yet</p>
        ) : (
            ...
            return (
              <div key={comment.id} className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-[#e2e8f0] leading-relaxed mt-0.5 whitespace-pre-wrap break-words">
      ...
      <div className="px-4 pb-4 pt-3">
        <div className="relative">
          ...
          <textarea
            ...
            rows={2}
            className="w-full px-3 py-2 pr-10 rounded-lg bg-[#0a1628] border border-[#2d3a52] text-white text-sm placeholder-[#64748b] focus:outline-none focus:border-[#d4af37]/50 resize-none transition-colors"
          />
          <button
            onClick={handleSubmit}
            disabled={!draft.trim() || submitting}
            className="absolute right-2 bottom-2.5 p-1.5 rounded-md text-[#64748b] hover:text-[#d4af37] disabled:opacity-30 disabled:hover:text-[#64748b] transition-colors"
        ...
        <p className="text-[10px] text-[#64748b] mt-1">
          <kbd className="px-1 py-0.5 rounded bg-[#2d3a52] text-[#94a3b8] font-mono">⌘↵</kbd> to send
        </p>
      </div>
    </div>
  );
