...
import { TendersInEvalCard } from './cards/TendersInEvalCard';
...
// uses only the four common cells in row 1.
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-12 gap-4 xl:gap-5 auto-rows-min">
        {/* Row 1 — common four (3 cols each on xl, 1 col each on md, full on mobile) */}
        <TasksCard
        ...
        />
        <ProcurementCard
          items={data.critical_procurement}
          href={hrefs.procurement}
          accent={accent}
          className="xl:col-span-3"
        />
        <TendersInEvalCard
          items={data.evaluation_tenders}
          href={hrefs.tendersInEval}
          accent={accent}
          className="xl:col-span-3"
        />
