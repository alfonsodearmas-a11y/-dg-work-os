...
export function GWIAnalysisPanel() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-20"><Spinner className="border-cyan-400" /></div>;
  }

  if (!analysis) {
    return <div className="card-premium p-8 text-center"><p className="text-navy-600">No GWI records found. Upload a GWI pending applications file first.</p></div>;
  }
  ...
  return (
    <div className="space-y-6">
          ...
          {analysis.regions.map(region => (
            <div key={region.region}>
              <button
                ...
                className="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-navy-900/50 text-sm transition-colors"
              >
                <div className="flex items-center gap-2">
                  {expandedRegion === region.region ? <ChevronDown className="h-3.5 w-3.5 text-cyan-400" /> : <ChevronRight className="h-3.5 w-3.5 text-navy-600" />}
                  <span className="text-white font-medium">{region.region}</span>
                  <span className="text-navy-600">{region.count} apps</span>
                </div>
              ...
              {expandedRegion === region.region && region.districts.length > 0 && (
                <div className="ml-8 mb-2 space-y-1">
                  {region.districts.map(d => (
                    <div key={d.district} className="flex items-center justify-between px-3 py-1.5 text-xs text-slate-400">
            ...
            <table className="w-full text-sm" aria-label="GWI community clusters">
              ...
              <tbody>
                {analysis.communityClusters.map(c => (
                  <tr key={c.village} className="border-b border-navy-800/50">
                    ...
                    <td className="py-2.5 pl-3 text-right text-slate-400">{c.avgDays}d</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="card-premium p-4 md:p-6">
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-sm" aria-label="GWI pending applications">
            ...
            <tbody>
              {loadingRecords ? (
                <tr><td colSpan={7} className="py-12 text-center"><Spinner className="border-cyan-400 mx-auto" /></td></tr>
              ) : records.length === 0 ? (
        ...
        <div className="md:hidden space-y-2">
          {loadingRecords ? (
            <div className="py-12 text-center"><Spinner className="border-cyan-400 mx-auto" /></div>
          ) : records.length === 0 ? (
      ...
      <div className="card-premium p-4 md:p-6">
        ...
        {!generatingAI && !deepAnalysis && (
          <p className="text-sm text-navy-600 py-4">Click Generate to create an AI-powered deep analysis of GWI pending applications.</p>
        )}
  ...
  return (
    <>
      <tr onClick={onToggle} className="border-b border-navy-800/50 hover:bg-navy-900/50 cursor-pointer transition-colors">
        ...
        <td className="py-3 px-3 text-slate-400">{r.district || '—'}</td>
        <td className="py-3 px-3 text-slate-400 truncate max-w-[150px]">{r.villageWard || '—'}</td>
