...
import { ProgressBar, DeadlineBadge } from './shared';
...
function ContractorCell({ contractors }: { contractors: string[] }) {
  ...
  return (
    <span className="text-sm text-white" title={contractors.join(', ')}>
      <span className="truncate">{contractors[0]}</span>
      {contractors.length > 1 && (
        <span className="ml-1 text-xs text-gold-500 font-medium">
          +{contractors.length - 1}
        </span>
      )}
    </span>
...
function SortHeader({
  ...
  label: string; field: string; sort: SortConfig; onSort: (field: string) => void; className?: string;
}) {
...
export function MinistryProjectTable({
  ...
  projects: OversightProject[]; loading: boolean; sort: SortConfig;
  onSort: (field: string) => void; onSelectProject: (p: OversightProject) => void;
  page: number; totalPages: number; total: number; onPageChange: (p: number) => void; isMobile: boolean;
}) {
  if (loading) {
    return (
  ...
  if (projects.length === 0) {
    return (
      <div className="bg-navy-900 border border-navy-800 rounded-xl p-8 text-center">
        <p className="text-navy-600 text-sm">No delayed projects found</p>
      </div>
    );
  }
          ...
          <div className="divide-y divide-navy-800/50">
            {projects.map((p) => (
              <div
                ...
                className="p-3 cursor-pointer hover:bg-navy-900/40"
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <DeadlineBadge endDate={p.project_end_date} />
                </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-navy-800">
              <tr>
                <SortHeader label="Agency" field="agency" sort={sort} onSort={onSort} className="w-20" />
                <SortHeader label="Project Name" field="name" sort={sort} onSort={onSort} />
                <SortHeader label="Region" field="region" sort={sort} onSort={onSort} className="w-20" />
                <SortHeader label="Contract Value" field="value" sort={sort} onSort={onSort} className="w-32" />
                <th className="px-3 py-2.5 text-left text-xs font-medium text-navy-600 uppercase tracking-wider">Contractor(s)</th>
                <SortHeader label="End Date" field="end_date" sort={sort} onSort={onSort} className="w-28" />
                <th className="px-3 py-2.5 text-left text-xs font-medium text-navy-600 uppercase tracking-wider w-28">Deadline</th>
                <SortHeader label="Completion" field="completion" sort={sort} onSort={onSort} className="w-32" />
              </tr>
            </thead>
            <tbody className="divide-y divide-navy-800/50">
              {projects.map((p) => (
                <tr
                  key={p.id}
                  onClick={() => onSelectProject(p)}
                  className="hover:bg-navy-900/40 cursor-pointer transition-colors"
                >
                  ...
                  <td className="px-3 py-2.5"><DeadlineBadge endDate={p.project_end_date} /></td>
