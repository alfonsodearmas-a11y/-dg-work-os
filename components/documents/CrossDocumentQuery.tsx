...
export function CrossDocumentQuery() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Answer */}
      {answer && (
        <div className="p-5 rounded-xl bg-[#d4af37]/10 border border-[#d4af37]/20 space-y-4">
          ...
          {sources.length > 0 && (
            <div className="pt-3 border-t border-[#d4af37]/20">
              <p className="text-xs text-[#64748b] mb-2 font-medium">Sources:</p>
              <div className="flex flex-wrap gap-2">
                {sources.map((source) => (
                  <Link
                    key={source.id}
                    href={`/documents/${source.id}`}
                    className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#1a2744] border border-[#2d3a52] hover:border-[#d4af37]/30 text-xs text-[#94a3b8] hover:text-white transition-colors"
                  >
                    <FileText className="h-3 w-3 text-[#d4af37]" />
                    {source.title}
                    {source.agency && (
                      <span className="text-[#d4af37]/60">({source.agency})</span>
                    )}
                  </Link>
                ))}
              </div>
