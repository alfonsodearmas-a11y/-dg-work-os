...
export function AnalyticsAgencyBreakdown({ packages }: Props) {
  ...
  const rows = useMemo(() => {
    ...
    for (const pkg of packages) {
      ...
      const entry = map.get(code) || { count: 0, days: [] };
  ...
  return (
    <div className="card-premium p-5 h-full flex flex-col">
      ...
      <div className="space-y-1 flex-1">
        {sorted.map((row) => (
          <div key={row.code} className="grid grid-cols-[1fr_60px_60px] gap-2 items-center px-1 py-2 rounded-lg hover:bg-navy-900/50 transition-colors">
